    ...
    axios({
        ...
        if (chat) {
            if (chat.participants[0]._id.toString() == globalProfile._id.toString()) {
                meImage = chat.participants[0].profilePicture ? `${baseURL}/uploads/${chat.participants[0].profilePicture}` : avatar
                friendImage = chat.participants[1].profilePicture ? `${baseURL}/uploads/${chat.participants[1].profilePicture}` : avatar
            } else {
                meImage = chat.participants[1].profilePicture ? `${baseURL}/uploads/${chat.participants[1].profilePicture}` : avatar
                friendImage = chat.participants[0].profilePicture ? `${baseURL}/uploads/${chat.participants[0].profilePicture}` : avatar
            }
